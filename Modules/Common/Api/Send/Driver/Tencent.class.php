...
/**
 * Class Tencent
 * @package Common\Api\Send\Driver
 * 腾讯云短信平台
 */
class Tencent extends \Common\Api\Send\Sms{
...
//        $result = $singleSender->send(0, "86", $phoneNumber2, "您的验证码为12312，若非本人操作请忽略！", "", "");
...
//        $rsp = json_decode($result);
        ...
        // 普通群发
        $multiSender    = new SmsMultiSender($this->username, $this->password);
        $result         = $multiSender->send(0, "86", explode(',', $this->phone), $this->msg, "", "");
        $resultObj      = json_decode($result);
        if($resultObj->result == 0){
            return true;
        } else {
            //M('Feedback')->data(array('content'=>"发送失败，失败原因：".$resultObj->errmsg))->add();
            return "发送失败，失败原因：" . $resultObj->errmsg;
        }
    }
}
