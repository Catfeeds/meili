 ...
 * 黑暗中的武者
 */
class OrderInfoService extends FrontBaseService {
    ...
    function orderList($custom_param = array()) {
        ...
        //基础字段
        $param['field']     = 'oi.id order_id,oi.order_sn,oi.pay_amounts,oi.status,oi.is_comm';
        ...
        $result = D('FrontC/OrderInfo')->getList($param);
        //处理数据
    ...
    function getOrderGoods($order_id, $group = 0) {
        if(empty($group))
            return M('OrderGoods')->where(array('order_id'=>$order_id))->field('goods_id,goods_name,goods_sn,price,number,cover')->select();
        else
            return M('OrderGoods')->where(array('order_id'=>$order_id))->group('goods_id')->field('goods_id,goods_name,goods_sn,price,number,cover')->select();
    }
    ...
    function orderDetail($custom_param = array()) {
        ...
        if(!empty($custom_param))
            $param = $this->customParam($param, $custom_param);
        //调用数据模型层方法获取数据
        $order = D('FrontC/OrderInfo')->findRow($param);
        if(!$order)
            return array();
        //时间处理
    ...
    function checkStatus($order_id, $actionable = array(), $order = array()) {
        ...
        //判断当前状态是否在可执行当前操作的状态数组中 不存在则不能进行操作
        if(!in_array(M('OrderInfo')->where(array('id'=>$order_id))->getField('status'), $actionable))
            return false;
        else
            return true;
    }
    ...
    function status_2($status, $order = array()) {
        if(empty($order))
            return '待发货';
        if($status <= 2)
            return array('active' => 1, 'title' => '待发货', 'time' => '', 'info' => '');
        if($status > 2)
            return array('active' => 0, 'title' => '已发货', 'time' => timestamp2date($order['pay_time']), 'info' => '');
    }
    function status_3($status, $order = array()) {
        if(empty($order))
            return '待收货';
        if($status <= 3)
            return array('active' => 1, 'title' => '待收货', 'time' => '', 'info' => '');
        if($status > 3)
            return array('active' => 0, 'title' => '已完成', 'time' => timestamp2date($order['arrived_time']), 'info' => '');
    }
