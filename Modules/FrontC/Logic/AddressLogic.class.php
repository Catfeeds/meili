 ...
 * 黑暗中的武者
 */
class AddressLogic extends FrontBaseLogic
{
    ...
    {
        //用户信息
        $param['where']['m_id'] = $request['m_id']; //用户ID
        //是否只返回默认地址
        if (!empty($request['default']))
            $param['where']['is_default'] = 1; //返回默认地址
        if (!empty($request['adr_id']))
            $param['where']['id'] = $request['adr_id']; //返回默认地址
        //获取列表  返回数据
        return D('FrontC/Address', 'Service')->getList($param);
    }
    ...
    function getRow($request = array())
    {
        ...
        $param['where']['id'] = $request['adr_id'];
        //地址信息
        $list = D('FrontC/Address', 'Service')->getList($param);
        //是否查到信息
        if (empty($list[0]))
            return $this->setLogicInfo('未查到地址信息！', false);
        return $list[0];
    }
    ...
    function setDefault($request = array())
    {
        if (empty($request['adr_id']))
            return $this->setLogicInfo('参数错误！', false);
        //默认置0
        if (M('Address')->where(array('m_id' => $request['m_id']))->setField('is_default', 0) === false) {
            return $this->setLogicInfo('系统繁忙，稍后重试！', false);
        }
        if (!M('Address')->where(array('m_id' => $request['m_id'], 'id' => $request['adr_id']))->setField('is_default', 1)) {
            return $this->setLogicInfo('系统繁忙，稍后重试！', false);
        }
