 ...
 * 黑暗中的武者
 */
class OrderInfoSerLogic extends FrontBaseLogic {
    ...
    function orderList($request = array()) {
        ...
        $param['where']['oi.flag'] = array('lt',3);
        $param['where']['oi.is_hidden'] = 0;
        return D('FrontC/OrderInfoSer', 'Service')->orderList($param);
    }
    ...
    function orderDetail($request = array()) {
        ...
        //查询该订单对应店铺信息
        $order_info=M('OrderInfoSer')->field('shop_id,flag,flag_id')->find($request['order_id']);
        //返回APP需要的商品或者套餐或者一卡通的封面图片和名称
        if($order_info['flag'] == 0){
            //单个商品
            $service_info=M('Service')->field('service_name,cover')->find($order_info['flag_id']);
            $cover_path=M('File')->where(array('id'=>$service_info['cover']))->getField('abs_url');
            $display_name=$service_info['service_name'];
            $display_cover=$cover_path;

        }elseif ($order_info['flag'] == 1){
            //套餐
            $package_info=M('Package')->field('package_name,cover')->find($order_info['flag_id']);
            $cover_path=M('File')->where(array('id'=>$package_info['cover']))->getField('abs_url');
            $display_name=$package_info['package_name'];
            $display_cover=$cover_path;

        }elseif ($order_info['flag'] == 2){
            //一卡通
            $card_info=M('Card')->field('card_name,cover')->find($order_info['flag_id']);
            $cover_path=M('File')->where(array('id'=>$card_info['cover']))->getField('abs_url');
            $display_name=$card_info['card_name'];
            $display_cover=$cover_path;

        }elseif ($order_info['flag'] == 3){
            //团购商品
            $group_service_id=M('ActivityGroupList')->where(array('id'=>$order_info['flag_id']))->getField('group_service_id');
            $service_id=M('ActivityGroupService')->where(array('id'=>$group_service_id))->getField('service_id');
            //单个商品
            $service_info=M('Service')->field('service_name,cover')->find($service_id);
            $cover_path=M('File')->where(array('id'=>$service_info['cover']))->getField('abs_url');
            $display_name=$service_info['service_name'];
            $display_cover=$cover_path;
        }
        $shop_name=M('Shop')->where(array('id'=>$order_info['shop_id'],'status'=>1))->getField('name');
        //订单ID查询
        $param['where']['oi.id'] = $request['order_id'];
        $result=D('FrontC/OrderInfoSer', 'Service')->orderDetail($param);
        $result['shop_id']=$order_info['shop_id'];
        $result['shop_name']=$shop_name;
        $result['display_name']=$display_name;
        $result['display_cover']=$display_cover;
        return $result;
    }
    ...
    function cancelOrder($request = array()) {
        ...
        //获取订单信息
        $order = M('OrderInfoSer')->where(array('m_id'=>$request['m_id'],'id'=>$request['order_id']))->field('pay_status,status')->find();
        //验证实时状态 是否可进行此操作
        ...
        //修改订单信息
        if(!M('OrderInfoSer')->where(array('id'=>$request['order_id']))->data($data)->save()) {
...
//            D('FrontC/Finance', 'Service')->addItgLog($request['m_id'], 1, 4, C('COMMENT_INTEGRAL'));
    ...
    function applyRefund($request = array()) {
        ...
        //获取订单信息
        $order = M('OrderInfoSer')->where(array('m_id'=>$request['m_id'],'id'=>$request['order_id']))->field('receiving_time,status')->find();
        //验证实时状态 是否可进行此操作
        if(!D('FrontC/OrderInfoSer', 'Service')->checkStatus(0, array(2,3), $order)) {
            return $this->setLogicInfo('当前状态不能进行此操作！', false);
        }
...
//        }
    ...
    function cancelRefund($request = array()) {
        ...
        //获取订单信息
        $order = M('OrderInfoSer')->where(array('m_id'=>$request['m_id'],'id'=>$request['order_id']))->field('before_status,status')->find();
        //验证实时状态 是否可进行此操作
    ...
    public function delOrder($request = array())
    {
        ...
        //获取订单信息
        $order = M('OrderInfoSer')->where(array('m_id'=>$request['m_id'],'id'=>$request['order_id']))->field('before_status,status,flag,flag_id')->find();
        //验证实时状态 是否可进行此操作
        if(!D('FrontC/OrderInfoSer', 'Service')->checkStatus(0, array(4,8,10),$order)) {
            return $this->setLogicInfo('当前状态不能进行此操作！', false);
        }
        //判断是否是团购订单 如果是团购订单则需要删除该团购
        if($order['flag'] == 3){
            if(!M('ActivityGroupList')->delete($order['flag_id'])){
                return $this->setLogicInfo('系统繁忙，稍后重试！', false);
            }
        }
