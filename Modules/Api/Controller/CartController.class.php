...
class CartController extends ApiBaseController
{
     ...
     * 该商城暂时不需要商品属性-goods_attr_desc(商品属性文字描述，格式：尺寸：100cm 颜色：黑色)
     */
    function addToCart()
    {
        ...
        if ($result === false)
            ...
            api_response('success', D('FrontC/Cart', 'Logic')->getLogicInfo());
    }
     ...
     * 详细描述：
     * 特别注意：
     * POST参数：*m_id(用户ID) cart_ids(购物车商品ID)
     */
    function getTotal()
    {
        api_response('success', '', D('FrontC/Cart', 'Service')->getTotalPrice(I('request.m_id'), I('request.cart_ids')));
    }
}