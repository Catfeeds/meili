...
class AdministratorLogic extends BmsBaseLogic {
    ...
    protected function afterUpdate($result, $request = array()) {
        //添加管理员之后 生成唯一标示
        if(empty($request['id'])) {
            call_procedure('_generate_code_', array($result, 'admin'));
        }
        return true;
    }
    ...
    function login($request = array()) {
        ...
        //查找该账号信息
        $info=M('Shop')->where(array('account'=>$request['account']))->field('id shop_id,account,password')->find();
        if(!empty($info['password'])){
            if(MD5($request['password']) == $info['password']){
...
//        }
     ...
     * 设置session
     */
    private function autoCZ($info) {
        /* 记录登录SESSION和COOKIES */
        $session = array(
            'a_id'            => $info['shop_id'],
            'shop_account'         => $info['account'],
        );
